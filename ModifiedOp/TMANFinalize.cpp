...
// Shape functions for AUTOSPLIT slicing of input "c" along dim 3.
// The output y (fp16, M channels) is split into chunks of 128 (= VecP) along dim 3.
// The input c (uint8, M*bits*sizeof(float) bytes) must be sliced proportionally.
// ratio = c_dim3 / y_dim3 = bits * sizeof(float), passed as an extra arg.
API_EXPORT QuickShape scaled_dim3_split_start(Replacement &rpx, Split_Context const &splitinfo, OpRef const &orig, int ratio)
{
    return QuickShape(0, 0, 0, (size_t)(splitinfo.start * ratio));
}

API_EXPORT QuickShape scaled_dim3_split_size(Replacement &rpx, Split_Context const &splitinfo, OpRef const &orig, int ratio)
{
    return QuickShape(
        orig.dim(rpx.graph(), 0),
        orig.dim(rpx.graph(), 1),
        orig.dim(rpx.graph(), 2),
        (size_t)(splitinfo.size * ratio)
    );
}
#endif
...
// AUTOSPLIT: match TMANLinear's tile split so each thread reads its own Tcm chunk.
// Output y is split on dim 3 with chunk_size=128 (=VecP, one tile of output channels).
// Input c is sliced on dim 3 with chunk_size = 128 * ratio, where ratio = bits * sizeof(float).
// bits=4: ratio=16, c_chunk=2048  (c_dim3 > 12 * y_dim3 distinguishes from bits=2 where ratio=8)
DEF_PACKAGE_OPTIMIZATION(
  EARLY,
  Op("TMANFinalize", "c", "group_size", "bits", "symmetric"),
  AND(GT(DIM_OF("*", 3), 128), GT(DIM_OF("c", 3), MUL(DIM_OF("*", 3), 12))),
  AUTOSPLIT(3, "I", 128,
    Op("TMANFinalize",
      AUTOSPLIT_SLICE("c",
        AUTOSPLIT_SHAPEFN_APPLY(scaled_dim3_split_start, "I", "c", 16),
        AUTOSPLIT_SHAPEFN_APPLY(scaled_dim3_split_size, "I", "c", 16)),
      "group_size", "bits", "symmetric")))

// bits=2: ratio=8, c_chunk=1024
DEF_PACKAGE_OPTIMIZATION(
  EARLY + 1,
  Op("TMANFinalize", "c", "group_size", "bits", "symmetric"),
  AND(GT(DIM_OF("*", 3), 128), LE(DIM_OF("c", 3), MUL(DIM_OF("*", 3), 12))),
  AUTOSPLIT(3, "I", 128,
    Op("TMANFinalize",
      AUTOSPLIT_SLICE("c",
        AUTOSPLIT_SHAPEFN_APPLY(scaled_dim3_split_start, "I", "c", 8),
        AUTOSPLIT_SHAPEFN_APPLY(scaled_dim3_split_size, "I", "c", 8)),
      "group_size", "bits", "symmetric")))
